...
/* for input buffering */
//主要是android的libc没有buffering，这个地方模拟出一个buffering的环境
//存储readline函数read到的数据
...
把Final response赋值到sp_response的finaResponse字段
*/
...
/*
如果sp_response为NULL那说明这是个unsolicited response，因为在onRequest里面
有这句: sp_response = at_response_new();
*/
    if (sp_response == NULL) {
        /* no command pending
        没有正在处理的command
        */
    	...
    	/*把sp_response的success变量赋值为1，并且把数据拷贝到sp_response的
	的finalResponse字段
    	*/
    ...
    //否则，可能是一些intermediate的response，处理之
    switch (s_type) {
        case NO_RESULT:
            handleUnsolicited(line);
            break;
        case NUMERIC: //只有一个intermediate的response，并且是以数字开头
            if (sp_response->p_intermediates == NULL
            	...
            	/*
			虽然我们的sp_response不为空，我们有正在处理的command，但
			也还是有可能在这个时间里面收到unsolicited response的
		*/
            ...
            break;
        case SINGLELINE: //只有一个intermediate response,并且是以指定的字符串开头(s_responsePrefix)
            if (sp_response->p_intermediates == NULL
		...
		*/
            ...
            break;
        case MULTILINE: //有多行的intermediate response
        //多行的response必须以指定的字符串开头(s_responsePrefix)
...
//response强制需要朝modem里面写入数据，一般是SMS的pdu
static int writeCtrlZ (const char *s)
{
    size_t cur = 0;
    size_t len = strlen(s);
...
//通常是在at_send_command_full_nolock出现问题的时候调用，清除掉
//当前正在执行的command的一些信息
...
/*
第一个参数是AT command
第二个参数是该AT command的AT response的前缀
第三个是AT response的值
