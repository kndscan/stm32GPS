...
void DrawChars(u16 x, u16 y, u16 fc, u16 bc, char* buf, unsigned char len)
{
	unsigned char i;
	for(i=0; i<len; i++)
	{	
		DrawChar(x+8*i, y, fc, bc, buf[i]);
	}
}
void DrawChar(u16 x, u16 y, u16 fc, u16 bc, short c)
{
	unsigned char i,j;
	unsigned short k;
	...
	else 
	{
		...
		{
			//if((hz16[k].Index[0]==*(s))&&(hz16[k].Index[1]==*(s+1)))
			//if((hz16[k].Index[1]<<8|hz16[k].Index[0]) == c)
			if(toUShort(hz16[k].Index[1], hz16[k].Index[0]) == c)
			{
				for(i=0;i<16;i++)
				{
					for(j=0;j<8;j++) 
